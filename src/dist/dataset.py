...
def init_dist_features(pocket_info, ligand_size):
    ...
    for i in range(protein_size):
        ...
        for j in range(protein_size):
            ...
            bb_dist[i, j, 0] = discretize_distance_numpy(d, 'b12') + 1
...
class DistDataset(Dataset):
    ...
    def __init__(self, split='train', cache_mode='memory', cache_dir='cache', no_dist_bins=12):
        self.cache = FileCache(cache_mode=cache_mode, cache_dir=cache_dir, dataset_name='cg_dist')
        self.split = split
    ...
    def collate_fn(batch):
        ...
        feat_bb_dist = batch[0]['bb_dist'].size(-1)
        ...
        bb_dist = torch.zeros(bsz, max_n, max_n, feat_bb_dist, dtype=torch.int64)
