...
from src.coords.dataset import parse_pocket, init_dist_to_coords_features
from src.coords.model import CoordsModel
from src.coords import config as coords_config

def _build_graph_from_features(features):
    """Create gnn graph compatible with CoordsModel from precomputed features."""
...
def run_coords_mode(pocket_structure, ligand_size: int, dist_matrix: torch.Tensor, coords_checkpoint: str = None, device: torch.device = None, seed: int = None):
    """High-level coords generation using the local CoordsModel and dataset utils."""
    if device is None:
    ...
    coords_checkpoint = pick_latest(['checkpoints/*coords*/*.ckpt'])
