...
def _attention(query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, biases: List[torch.Tensor]) -> torch.Tensor:
    ...
    # kv = torch.matmul(key, value) # [*, H, C_hidden, C_hidden]
...
class Attention(nn.Module):
    ...
    def _prep_qkv(self,
        ...
        # q_norm = torch.sqrt(torch.sum(q * q, dim=-1, keepdim=True) + 1e-8)
        # k_norm = torch.sqrt(torch.sum(k * k, dim=-1, keepdim=True) + 1e-8)
        # q = q / q_norm
        # k = k / k_norm
...
class TriangleAttention2(nn.Module):
    def __init__(
        self, c_in, c_hidden, direction='row', inf=1e9
    ):
        super(TriangleAttention2, self).__init__()
        ...
        self.linear_q = Linear(c_in, hidden, bias=False, init="normal")
        self.linear_k = Linear(c_in, hidden, bias=False, init="normal")
        self.linear_v = Linear(c_in, hidden, bias=False, init="normal")
        self.linear_gate = Linear(c_in, 1, init="gating")
        # self.linear_out = Linear(hidden, c_in, init="final")
        # self.gated_output = GatedOutput(c_in)
        self.linear_out = nn.Sequential(
            Linear(hidden, hidden, init="relu"),
            nn.SiLU(),
            Linear(hidden, c_in, init="final"),
        )
    ...
    def forward(self,
        ...
        q = self.linear_q(x)
        k = self.linear_k(x) * gate
        v = self.linear_v(x) * gate

        # l2 normalize
        # k = k / (torch.sqrt(torch.sum(k * k, dim=-1, keepdim=True) + 1e-8))
        # v = v / (torch.sqrt(torch.sum(v * v, dim=-1, keepdim=True) + 1e-8))

        w = torch.einsum("bijd,bjkd->bikd", k, v) / x.shape[1]
        w = (w + w.transpose(-2, -3)) / 2

        return self.linear_out(q * w)
...
class SequenceAttention(nn.Module):
    ...
    def __init__(self, c_m, c_z, c_hidden, no_heads, inf=1e9):
        ...
        self.linear_z_k = Linear(c_z, c_m, init="final")
        self.linear_z_gate = Linear(c_z, 1, init="gating")
        self.linear_z_q = Linear(c_m, c_m, init="final")
        self.linear_z_s = nn.Sequential(
            Linear(c_m, c_m, init="relu"),
            nn.SiLU(),
            Linear(c_m, c_m, init="final"),
        )
    ...
    def forward(self, seq, z, mask=None, chunk_size=None):
        ...
        z1 = torch.sum(self.linear_z_k(z * self.linear_z_gate(z)), dim=-2) / z.shape[-2]
        seq = seq + self.linear_z_s(self.linear_z_q(seq) * z1)
        ...
        q_h = q.transpose(-3, -2)
        k_h = k.transpose(-3, -2)
        # q_norm = torch.sqrt(torch.sum(q_h * q_h, dim=-1, keepdim=True) + 1e-8)
        # k_norm = torch.sqrt(torch.sum(k_h * k_h, dim=-1, keepdim=True) + 1e-8)
        # q_h = q_h / q_norm
        # k_h = k_h / k_norm
        scores_h = torch.matmul(q_h, k_h.transpose(-1, -2)) # [*, H, N_res, N_res]
        scores = scores_h.permute(0, 2, 3, 1) # [*, N_res, N_res, H]
...
class SequenceOuterProduct2(nn.Module):
    """
    Outer product for sequence embeddings.
    Replaces OuterProductMean for single sequence processing.
    """
    def __init__(self, c_m, c_z, c_hidden, eps=1e-3):
        super(SequenceOuterProduct2, self).__init__()
        ...
        self.layer_norm = LayerNorm(c_m)
        self.linear_1 = Linear(c_m, c_z)
        self.linear_2 = Linear(c_m, c_z)
        self.linear_gate1 = Linear(c_m, 1, init="gating")
        # self.linear_gate2 = Linear(c_m, 1, init="gating")
        # self.linear_out = Linear(c_z, c_z, init="final")
        self.linear_out = nn.Sequential(
            Linear(c_z, c_z, init="relu"),
            nn.SiLU(),
            Linear(c_z, c_z, init="final"),
        )
        # self.linear_out = Linear(c_hidden ** 2, c_z, init="final")
        # self.linear_out = Linear(c_hidden, c_z, init="final")
    ...
    def forward(self, seq, z, mask=None, chunk_size=None):
        ...
        seq = self.layer_norm(seq)
        gate = self.linear_gate1(seq)
        
        mask = mask.unsqueeze(-1)
        # seq = self.linear_1(seq) * self.linear_gate(seq)
        a = self.linear_1(seq) * mask * gate
        b = self.linear_2(seq) * mask * gate
        
        # Outer product: [*, N_res, N_res, C_hidden, C_hidden]
        # outer = torch.einsum("...ia,...jb->...ijab", a, b)
        
        # Alternative strategy: broadcast concat along pair dimensions
        seq_len = seq.shape[-2]
        a_expanded = a.unsqueeze(2).expand(-1, seq_len, seq_len, -1)
        b_expanded = b.unsqueeze(1).expand(-1, seq_len, seq_len, -1)
        outer = a_expanded + b_expanded
        
        # Flatten and project
        # outer = outer.reshape(outer.shape[:-2] + (-1,))
        outer = self.linear_out(outer * z)
        
        # Normalize
        # norm = torch.einsum("...ia,...ja->...ij", mask, mask)
        # outer = outer / (self.eps + norm.unsqueeze(-1))
        
        return (outer + outer.transpose(-2, -3)) / 2

class GatedOutput(nn.Module):
    def __init__(self, c_hidden):
        super(GatedOutput, self).__init__()
        
        self.c_hidden = c_hidden
        
        self.linear_out = Linear(c_hidden, c_hidden, init="gating")
        self.sigmoid = nn.Sigmoid()
    
    def forward(self, x):        
        z = self.sigmoid(self.linear_out(x))
        return x * z
...
class EvoformerBlockCore(nn.Module):
    def __init__(
        ...
        self.seq_outer_product = SequenceOuterProduct2(
            c_m,
            c_z,
            c_hidden_opm,
        )
        ...
        self.tri_att_row = TriangleAttention2(
            c_z,
            c_hidden_pair_att,
            inf=inf,
            direction='row',
        ...
        self.linear_z_k = Linear(c_z, c_z, init="final")
        self.linear_z_q = Linear(c_z, c_z, init="final")
        self.linear_z_s = Linear(c_z, c_z, init="final")
        # self.gated_output = GatedOutput(c_z)
        self.linear_z_s = nn.Sequential(
            Linear(c_z, c_z, init="relu"),
            nn.SiLU(),
            Linear(c_z, c_z, init="final"),
        ...
        self.linear_z_k2 = Linear(c_z, c_z, init="final")
        self.linear_z_q2 = Linear(c_z, c_z, init="final")
        # self.gated_output2 = GatedOutput(c_z)
        # self.linear_z_s2 = Linear(c_z, c_z, init="final")
        self.linear_z_s2 = nn.Sequential(
            Linear(c_z, c_z, init="relu"),
            nn.SiLU(),
            Linear(c_z, c_z, init="final"),
        )
    ...
    def forward(
        ...
        z = z + self.seq_outer_product(
            seq, z, mask=seq_mask, chunk_size=chunk_size
        ) # [*, N_res, N_res, C_z]

        z = z + self.tri_att_row(z)

        z1 = torch.sum(self.linear_z_k(z), dim=-2, keepdim=True) / z.shape[-2]
        # z1 = torch.sum(self.linear_z_k(z), dim=-2, keepdim=True) / z.shape[-2]
        z1 = self.linear_z_s(self.linear_z_q(z) * z1)
        z1 = (z1 + z1.transpose(-2, -3)) / 2
        z = z + z1

        z2 = torch.sum(self.linear_z_k2(z), dim=[-2, -3], keepdim=True) / z.shape[-2] / z.shape[-3]
        z2 = self.linear_z_s2(self.linear_z_q2(z) * z2)
        z2 = (z2 + z2.transpose(-2, -3)) / 2
        z = z + z2

        # z = z + self.tri_att_column(z)
...
class EvoformerStack(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # self.seq_outer_product = SequenceOuterProduct(c_m, c_z, c_hidden_opm)
    ...
    def forward(self,
        ...
        s = self.linear(seq)
        # z = self.seq_outer_product(seq, mask=seq_mask)
