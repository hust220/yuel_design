...
from src.e2e.app import run_e2e_mode, save_e2e_pdb, save_e2e_trajectory
...
def test_e2e_mode(device):
    ...
    try:
        final_prediction, chain, pocket_info = run_e2e_mode(
    ...
    save_e2e_pdb(final_prediction, pocket_info, predicted_pdb_path)

    trajectory_path = os.path.join(output_dir, "trajectory.pdb")
    print(f"Saving trajectory to {trajectory_path}")
    save_e2e_trajectory(chain, pocket_info, trajectory_path)

    print(f"✓ E2E mode test passed! Generated coordinates with shape: {final_prediction.shape}")
    print(f"  Chain length: {len(chain)}")
    print(f"  Pocket atoms: {len(pocket_info['coords'])}")


@pytest.mark.parametrize("ligand_size", [10, 20, 30])
def test_e2e_mode_different_sizes(moad_sample, device, ligand_size):
    pocket_id, pocket_pdb, ligand_mol, ligand_name = moad_sample
    print(f"Testing E2E mode with ligand size {ligand_size} for sample: {pocket_id}")
    ...
    try:
        ...
        pytest.fail(f"E2E mode test failed for ligand size {ligand_size}: {e}")
