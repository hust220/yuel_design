...
def init_ligand_coords_features(int_index, receptor_atoms, ligand_fixed_atoms, ligand_size):
    ...
    # Create mask: 1 only for CA atoms, 0 for all other atoms
    mask = np.zeros(n_total, dtype=np.int64)
...
def build_edges(int_index, receptor_atoms, ligand_size):
    """Build graph edges with full connectivity.
    Creates a fully connected graph with interaction information as edge features.
    ...
    Returns:
        tuple: (edge_index, edge_attr)
            edge_index: numpy array of shape (2, n_edges) with edge indices
            edge_attr: numpy array of shape (n_edges, 2) with [is_same_residue, is_interaction] features
    """
    ...
    # Convert int_index to a set for fast lookup
    int_index_set = set(int_index)
    # Also add reverse pairs for bidirectional edges
    ...
    for i in range(n):
        for j in range(i+1, n):
            ...
            # Build edge attributes: is_same_residue and is_interaction
            res_i, res_j = res_indices[i], res_indices[j]
            is_same_residue = 1 if (res_i == res_j and res_i != -1) else 0
            is_interaction = 1 if (i, j) in int_index_set or (j, i) in int_index_set else 0
            edge_attr.append([is_same_residue, is_interaction])
            edge_attr.append([is_same_residue, is_interaction])
...
class ContDataset(Dataset):
    ...
    def features_to_graph(features):
        ...
        Args:
            features: dict with keys 'x', 'h', 'edge_index', 'edge_attr', 'mask'
        ...
        g.edata['edge_attr'] = torch.tensor(features['edge_attr'], dtype=torch.float32)
    ...
    def collate_fn(batch_data):
        """Collate function for LigandDataset using custom batch functionality"""
