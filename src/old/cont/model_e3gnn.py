...
class ContModel(torch.nn.Module):
    def __init__(self, **kwargs):
        ...
        # E3GNN initialization - accepts kwargs dict
        e3gnn_kwargs = {
            'hidden_nf': hidden_nf,
            'n_blocks': self.n_blocks,
            'in_node_features': in_node_features + 2,  # +1 for mask (concatenated in _egnn_forward)
            'in_edge_features': in_edge_features,  # 2 for [is_same_residue, is_interaction]
        }
        self.e3gnn = E3GNN(e3gnn_kwargs)        
    ...
    def _egnn_forward(self, xt, t, graph):
        ...
        # E3GNN forward: forward(h, x, e, edge_index) -> (h, dx)
        # E3GNN internally computes distance and concatenates to edge_attr
        h_out, dx = self.e3gnn.forward(
            h=h,
            x=x_in,
            e=edge_attr,
    ...
    def forward(self, graph, training=None):
        ...
        eps_hat = self._egnn_forward(xt, t, graph) * (1.0 - fix_mask[:, None])
        n_nodes = (1.0 - fix_mask).sum()
        loss = ((eps_t - eps_hat) ** 2).sum() / n_nodes
    ...
    def sample_chain(self, graph, keep_frames=None):
        ...
        for s_step in tqdm(reversed(range(0, self.T)), desc="Diffusion sampling", total=self.T):
            ...
            x = alpha_t * x0 * fix_mask + x * sample_mask
