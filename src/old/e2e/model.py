...
class E2EModel(torch.nn.Module):
    ...
    def _egnn_forward(self, xt, t, graph):
        ...
        e = self.embedding_edge(edge_attr.float())
        x = xt
    ...
    def forward(self, graph, training=None):
        ...
        eps_t = torch.randn_like(x0) * free_mask[:, None]
    ...
    def sample_chain(self, graph, keep_frames=None):
        ...
        anchor_mask = graph.ndata['anchor_mask']
        free_mask = 1.0 - anchor_mask
        anchor_mean = x[anchor_mask == 1].mean(dim=0, keepdim=True)
        x = x - anchor_mean
        xt = x * anchor_mask[:, None] + torch.randn_like(x) * free_mask[:, None] # [N, F]
        ...
        for s_step in reversed(range(0, self.T)):
            ...
            xt = alpha_t * x * anchor_mask[:, None] + xt * free_mask[:, None]
            xt = self.sample_p_zs_given_zt(s, t, xt, graph)

            chain.append(xt + anchor_mean)

        gamma_0 = self.gamma(torch.zeros(1, device=x.device))
        alpha_0 = alpha(gamma_0, x)
        xt = alpha_0 * x * anchor_mask[:, None] + xt * free_mask[:, None]
        xt = self.sample_p_xh_given_z0(xt, graph)
        chain.append(xt + anchor_mean)
