#!/usr/bin/env python
"""
Convert all SDF files in diffsbdd folder to mol2 files.
...
def main():
    parser = argparse.ArgumentParser(description='Convert SDF files in diffsbdd folder to mol2 files.')
    ...
    diffsbdd_dir = script_dir / "diffsbdd"
    
    if not diffsbdd_dir.exists():
        print(f"Error: Directory {diffsbdd_dir} does not exist")
        sys.exit(1)
    
    # Find all SDF files
    sdf_files = sorted(diffsbdd_dir.glob("*.sdf"))
    
    if not sdf_files:
        print(f"No SDF files found in {diffsbdd_dir}")
        sys.exit(0)
    
    print(f"Found {len(sdf_files)} SDF files")
    print(f"Output directory: {diffsbdd_dir}")
    if args.force:
    ...
    for sdf_file in sdf_files:
        output_mol2 = diffsbdd_dir / f"{sdf_file.stem}.mol2"
