...
class SlurmDockingWorker:
    def __init__(self, db_params, batch_size=50, n=None, table_name=None):
    ...
    def _process_molecule(self, mol_id):
        ...
        try:
            ...
            with self._connect() as conn:
                cursor = conn.cursor()
                cursor.execute(f"""
                    SELECT receptor, pocket, ligand FROM {self.table_name}
                    WHERE id = %s
                """, (mol_id,))
                r = cursor.fetchone()
                receptor = r[0]
                ...
                cursor.execute(f"""
                    SELECT mol FROM ligands
                    WHERE name = %s
                """, (ligand,))
                ligand_row = cursor.fetchone()
                ligand_mol = ligand_row[0]

                cursor.execute(f"""
                    SELECT mol FROM ligands
                    WHERE name = %s
                """, (pocket,))
                pocket_row = cursor.fetchone()
                pocket_mol = pocket_row[0]
                ...
                receptor_pdb = receptor_row[0]

                cursor.close()

            if not receptor_pdb or not pocket_mol or not ligand_mol:
            ...
            with tempfile.TemporaryDirectory(dir=str(docking_dir), prefix=f"dock_{mol_id}_") as tmp_dir:
                ...
                # Write ligand file
                ligand_file = tmp_path / "ligand.mol"
                with open(ligand_file, 'wb') as f:
                    f.write(ligand_mol.tobytes() if hasattr(ligand_mol, 'tobytes') else ligand_mol)
...
if __name__ == "__main__":
    ...
    worker = SlurmDockingWorker(db_params, args.batch, args.n, args.table)
    worker.run()